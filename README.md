...
## 安装到你的 Agent（Codex / zcode / Claude Code / Cline / DSH）

见 [integrations/README.md](integrations/README.md)：仓库即 Claude Code 插件市场（marketplace add 一条命令）、
Codex 一条命令注册 MCP、zcode/DSH 技能目录一步安装、Cline 配置片段。
...
```bash
# 2D 通道真跑（本机已验证通过，约 1 分钟；Windows cmd 用 ^ 换行）
...
```
L3 接入层  tools/mcp_server.py（stdio MCP：aero_doctor/aero_run/aero_optimize/aero_pipeline/aero_results）
          + skills/aero-fluent（流程 skill：只填参数不写语法 + 报错知识库 + 失败决策表 + 人工关口）
             ↑ agent（VSCode/Codex/zcode/Claude Code/Cline/DSH）自然语言驱动
L2 自动化层 run_pipeline.py CLI（doctor/run/optimize/pipeline/journal/inspect）
...
docs/                      # external_flow_domain_and_mesh.md（域尺寸/回流/加密/破面官方引文）
                           # official_examples_translation.md（官方示例翻译对照表）
...
configs/                   # demo_channel.json（已验证）/ case_airplane.json（E10 参数）
                           #   / demo_baseline.json（M1.5 基线）
meshes/                    # channel2d.msh / channel2d_lam.msh（生成器产物）、channel_domain.stl
                           #   （三命名面区域，watertight 探针用）、channel_box.stl
integrations/              # 各 agent（Codex/zcode/Claude Code/Cline/DSH）安装指南
tools/                     # mock_fluent.py / selftest.py / baseline_check.py /
                           #   degradation_drill.py / make_demo_msh.py / make_demo_stl.py / mcp_server.py
scripts/                   # spaceclaim_box_domain.py（M2，IronPython 内层）
...
| M3 网格自动化 | ◐ | datamodel 路线骨架**真机验证**（InitializeWorkflow/TaskObject 可用）；**v222 版本边界实证**：WTM 导入不支持 STL（官方报错），需 .scdoc/.x_t/.step CAD 输入（与 M2 产物衔接）；演示管线用参数化网格生成器替代；STL 支持升版后解锁 |
| M4 求解+后处理 | ✅ | 求解→CSV/JSON；受力解析三格式；残差/收敛/质量/流量守恒/回流告警解析 |
| M5 优化闭环 | ✅ | optuna 两级 + 去重 + 内置降级引擎；mock 响应面实证收敛 |
| M6 接入层完善 | ✅ | 自研 fluent-mcp 5 工具（兼容 mcp 1.x/2.x）+ aero-fluent skill（报错知识库与反馈闭环指引）+ 校准记录 |
| 官方示例翻译 | ✅ | external_compressible_flow（跨音速机翼）与 ahmed_body_watertight（官方网格工作流）翻译件 + 对照表；far-field BC/BOI/边界层参数已进生成器；**官方任务链语法与本机探针骨架互证一致** |
| M7 全流程打通/降级演练 | ✅ | `run_pipeline.py pipeline` 一键（几何→网格→求解→汇总报告）；降级演练 5/5（L3 MCP→L2 CLI→L1 手工 journal→L0 指引） |
